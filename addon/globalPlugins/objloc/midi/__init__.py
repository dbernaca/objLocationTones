"""
Modified version of pygame.midi module to work as standalone package plus some extra for high-level stuff.
Modifications are for now only output related.
The original is a pygame module for interacting with midi input and output.
Note: All licenses for it and its dependencies are as in original module.
Modifications by Dalen Bernaca in 2025 under GPL

The midi package can send output to midi devices, and get input
...
def init():
    """initialize the midi package
    midi.init(): return None

    Call the initialisation function before using the midi package.
...
def _check_init():
    if not _module_init():
        raise RuntimeError("midi package not initialised.")
